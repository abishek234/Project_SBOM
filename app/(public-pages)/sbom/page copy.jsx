                            ...
                            <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                    ...
                                    <div className="space-y-12 mb-12">
                                        ...
                                        <div className="flex flex-col md:flex-row gap-6">
                                            ...
                                            <div className="flex-1 bg-red-500/5 backdrop-blur-md border border-red-500/10 p-8 rounded-3xl">
                                                <div className="flex items-center justify-between mb-6">
                                                    <h3 className="text-red-400 font-black uppercase tracking-widest text-xs">Removed Packages</h3>
                                                    <div className="px-2 py-1 bg-red-500/20 text-red-400 rounded-md text-[10px] font-bold">-{(results?.removed || []).length}</div>
                                                </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-8 bg-white/5 border border-white/10 rounded-3xl relative overflow-hidden group">
                                        ...
                                        {/* Project-Wide Factor Breakdown */}
                                        <div className="mt-8 pt-8 border-t border-white/5">
                                            <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-4">Risk Exposure Summary</h4>
                                            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                                                {(() => {
                                                    const allPkgs = results?.components?.flatMap(c => c.packages) || [];
                                                    const factorLabels = [
                                                        { label: "Vulnerabilities", key: "Known CVE", color: "#ef4444" },
                                                        { label: "Outdated", key: "Outdated Version", color: "#f97316" },
                                                        { label: "Compliance", key: "Missing License", color: "#eab308" },
                                                        { label: "Community", key: "Low Popularity", color: "#a855f7" },
                                                        { label: "Maintenance", key: "No Recent Update", color: "#6366f1" }
                                                    ];

                                                    const aggregates = allPkgs.reduce((acc, p) => {
                                                        Object.entries(p.factor_points || {}).forEach(([k, v]) => {
                                                            acc[k] = (acc[k] || 0) + v;
                                                        });
                                                        return acc;
                                                    }, {});

                                                    const totalPoints = Object.values(aggregates).reduce((a, b) => a + b, 0) || 1;

                                                    return factorLabels.map((f) => {
                                                        const val = aggregates[f.key] || 0;
                                                        const pct = Math.round((val / totalPoints) * 100);
                                                        return (
