                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        {(() => {
                                            ...
                                            const criticals = allPkgs.filter(p => p.level === 'CRITICAL').length;
                                            const lows = allPkgs.filter(p => p.level === 'LOW' || p.level === 'SAFE').length;
                                            const outdated = allPkgs.filter(p => p.factor_points?.["No Recent Update"] > 0).length;
                                            const licenses = allPkgs.filter(p => p.factor_points?.["Missing License"] > 0).length;

                                            const milestones = [
                                                {
                                                    label: "Vulnerability Density",
                                                    value: `${criticals} Criticals`,
                                                    status: criticals > 0 ? 'alert' : 'safe',
                                                    desc: criticals > 0 ? "High concentration of exploitable CVEs." : "No critical CVEs identified."
                                                },
                                                {
                                                    label: "Maintenance Pulse",
                                                    value: `${outdated} Stale`,
                                                    status: outdated > (allPkgs.length * 0.3) ? 'warning' : 'safe',
                                                    desc: "Dependencies with no updates in 1+ years."
                                                },
                                                {
                                                    label: "Compliance Scan",
                                                    value: `${licenses} Issues`,
                                                    status: licenses > 0 ? 'warning' : 'safe',
                                                    desc: "Unlicensed or non-standard OSS detected."
                                                },
                                                {
                                                    label: "Project Stability",
                                                    value: `${Math.round((lows / (allPkgs.length || 1)) * 100)}% Safe`,
                                                    status: (lows / (allPkgs.length || 1)) < 0.5 ? 'warning' : 'safe',
                                                    desc: "Ratio of healthy vs risky packages."
                                                }
                                            ];
                                            ...
                                            return milestones.map((m, i) => (
                                                ...
                                                >
                                                    <div className="text-[9px] font-black uppercase tracking-widest text-gray-500">{m.label}</div>
                                                    <div className={`text-xl font-black ${m.status === 'alert' ? 'text-red-500' : m.status === 'warning' ? 'text-orange-500' : 'text-green-500'}`}>
                                                        {m.value}
                                                    </div>
