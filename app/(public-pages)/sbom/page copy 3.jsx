...
function SBOMContent() {
    ...
    const handleFileUpload = async (e) => {
        ...
        setStep('scanning');
        setScanProgress(0);
        setScanStatus('Connecting to backend...');
        ...
        try {
            ...
            const statusMessages = [
                "Initializing Genesis Intelligence Engine...",
            ...
            const interval = setInterval(() => {
                ...
                if (curProgress > 95) {
                    ...
                    setScanStatus(statusMessages[Math.floor((curProgress / 100) * statusMessages.length)]);
                }
            }, 800);
    ...
    if (!user) {
        return (
            <div className="min-h-screen pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto flex flex-col items-center justify-center">
                <SectionTitle title="Smart SBOM Generator" description="Please log in to access the scanning tools." />
                <button
    ...
    return (
        <>
            ...
            <div className="pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto">
                <SectionTitle title="Smart SBOM Dashboard" description={`Welcome back, ${user.full_name}`} />
                            ...
                            >
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    >
                                        ...
                                        <p className="text-gray-400 text-sm leading-relaxed">Upload a zip file to receive a detailed Security Bill of Materials (SBOM) and risk intelligence report.</p>
                                    </motion.div>
                                    ...
                                    >
                                        ...
                                        <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-purple-400 transition-colors">Version SBOM (Diff)</h3>
                                        <p className="text-gray-400 text-sm leading-relaxed">Compare two versions of your project to detect newly introduced risks and dependency updates.</p>
                                    </motion.div>
                                </div>
                        ...
                        {step === 'results' && results && (
                            <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-8 bg-white/5 border border-white/10 rounded-3xl relative overflow-hidden group">
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                                            <div className="flex flex-col gap-1">
                                                <div className="text-gray-400 font-medium uppercase tracking-widest text-xs">Security Intelligence</div>
                                                <h3 className="text-3xl font-black text-white">Overall Project Risk</h3>
                                            </div>
                                            <div className="flex items-center gap-6">
                                                <div className="text-right">
                                                    <div className={`text-6xl font-black tracking-tighter ${results.riskScore > 70 ? 'text-red-500' : results.riskScore > 30 ? 'text-orange-500' : 'text-green-500'}`}>
                                                        {results.riskScore}<span className="text-2xl opacity-50">/100</span>
                                                    </div>
                                                    <div className="text-gray-500 font-bold uppercase text-[10px]">Aggregated Score</div>
                                                </div>
                                    ...
                                    {/* Intelligence Milestones / Score Breakdown */}
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        {(() => {
                                            const allPkgs = results?.components?.flatMap(c => c.packages) || [];
                                            const factorLabels = [
                                                { label: "Vulnerability Density", key: "Known CVE", color: "#ef4444", desc: "Exploitable CVE density" },
                                                { label: "Maintenance Pulse", key: "Outdated Version", color: "#f97316", desc: "Stale/Legacy dependency aging" },
                                                { label: "Compliance Scan", key: "Missing License", color: "#eab308", desc: "License compliance & legal risk" },
                                                { label: "Project Stability", key: "Low Popularity", color: "#a855f7", desc: "Community adoption & trust" }
                                            ];

                                            // Calculate raw average points per factor across all packages
                                            const rawPoints = factorLabels.reduce((acc, f) => {
                                                const sum = allPkgs.reduce((s, p) => s + (p.factor_points?.[f.key] || 0), 0);
                                                acc[f.key] = sum / (allPkgs.length || 1);
                                                return acc;
                                            }, {});

                                            const rawTotal = Object.values(rawPoints).reduce((a, b) => a + b, 0) || 1;
                                            const scalingFactor = (results.riskScore || 0) / rawTotal;

                                            return factorLabels.map((f, i) => {
                                                const contribution = rawPoints[f.key] * scalingFactor;
                                                const displayVal = Math.round(contribution);
                                                const status = displayVal > 30 ? 'alert' : displayVal > 10 ? 'warning' : 'safe';
                                                ...
                                                return (
                                                    <motion.div
                                                        ...
                                                        className="p-4 bg-white/5 border border-white/10 rounded-2xl flex flex-col gap-2 group/milestone hover:bg-white/[0.08] transition-all"
                                                    >
                                                        <div className="text-[9px] font-black uppercase tracking-widest text-gray-500">{f.label}</div>
                                                        <div className={`text-xl font-black ${status === 'alert' ? 'text-red-500' : status === 'warning' ? 'text-orange-500' : 'text-green-500'}`}>
                                                            {displayVal} pts
                                                        </div>
                                                        <div className="text-[10px] text-gray-400 leading-tight opacity-0 group-hover/milestone:opacity-100 transition-opacity">
                                                            {f.desc}
                                                        </div>
                                                    </motion.div>
                                                );
                                        ...
                                        {(results?.components || []).map((comp, ci) => (
                                            <div key={ci} className="space-y-4">
                                                <div className="flex items-center gap-3 px-2">
                                                    ...
                                                    <div className={`text-sm font-black ${comp.riskScore > 70 ? 'text-red-500' : 'text-green-500'}`}>
